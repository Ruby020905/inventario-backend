...
import com.company.inventario.dao.IProductDao;
...
public class ProductServiceImpl implements IProductService {
		...
		try {
			...
			//search product by id
			 ...
			 if(product.isPresent()) {
				 ...
				 response.getProduct().setProducts(list);response.setMetadata("Respuesta ok", "00", "Producto encontrado");
			 } else {
	...
	public ResponseEntity<ProductResponseRest> deleteById(Long id){
		...
		try {
			...
			e.getStackTrace();
			response.setMetadata("respuesta nok", "-1", "Error al buscar eliminar por id");
			 return new ResponseEntity <ProductResponseRest>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		...
		try {
			...
			//search category to set in the product object
			 ...
			 if(productSearch.isPresent()) {
				 ...
				 productSearch.get().setType(product.getType());
				 productSearch.get().setDate(product.getDate());
